...
class StudentMarksSMFA01Controller extends \BaseController {
	...
	{
        return View::make('student_marks_SMF_A_01');
	}


    public function students_for_marks_smf_a_01(){
        //return Student::all();
    ...
    public function save_marks_for_smf_A_01(){
...
/*
        ...
        $has_row = DB::table('students_for_marks_smf_a_01')->where('san','=',$san)->get();
        if($has_row){
			...
			$row = DB::table('students_for_marks_smf_a_01')->where('san','=',$san)->first();
			...
			$mark_update = new StudentMarksSMFA01();
            $mark_update->san = $san;
	...
	public function to_word(){
		...
		//return Input::get('export_type');
			$student_data = DB::table('students_for_marks_smf_a_01')
			->leftJoin('application_scj','students_for_marks_smf_a_01.san','=','application_scj.san')
			->where('students_for_marks_smf_a_01.san','=',$san)->get(); 
		if($student_data){
    ...
    "Content-Disposition"=>"attachment;Filename=smf_".$student_data[0]->scj_number." & ".$student_data[0]->ls_student_number."_".Sentry::getUser()->first_name." ".Sentry::getUser()->last_name.".doc");
...
- The aims or purpose of the report<br>
- A very brief overview of the chosen corporation<br>
- A final paragraph that outlines the next section of the report<br>
  ...
  <tr>
    <td height="70">
	<strong>Theoretical/Conceptual Approach and Analysis</strong><br><br>
Clear explanation of the theoretical or conceptual approach that is to be used, its relevance for the purpose. <br>
Include any debates about, or criticisms and weaknesses of, the approach.<br>
Specify the material you are going to analyse, its source, and why it is relevant for this analysis.<br>
(Maximum 10 marks for each approach) 
...
For each analysis, specify the material to be analysed, its source, and why it is relevant for this particular analytical approach.
- For each analysis, systematically apply the approach and explanation of the candidate’s findings.
(Maximum 20 marks for each analysis)


</td>
    <td>&nbsp;</td>  <td>&nbsp;'.$student_data[0]->c3.'</td><td align="center">0.40</td>
    <td>&nbsp;'.($student_data[0]->c3*.4).'</td>
  </tr>
  <tr>
    <td height="70">
		<strong>Discussion and conclusion </strong><br><br>
...
Appearance and written expression
  ...
  <tr>
    ...
    <td>&nbsp;'.(($student_data[0]->c1*.1)+($student_data[0]->c2*.2)+($student_data[0]->c3*.4)+($student_data[0]->c4*.2)+($student_data[0]->c5*.1)).'</td>
  </tr>
  </table>
