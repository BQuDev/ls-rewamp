...
class StudentMarksRMBMMBAA01Controller extends \BaseController {
	...
	{
        return View::make('student_marks_RMBM_MBA_A_01');
	}


    public function students_for_marks_rmbm_mba_a_01(){
        //return Student::all();
        return DB::table('students')
            ->leftJoin('students_for_marks_rmbm_mba_a_01','students.san','=','students_for_marks_rmbm_mba_a_01.san')
    ...
    public function save_marks_for_RMBM_MBA_A_01(){
...
/*
        ...
        $has_row = DB::table('students_for_marks_rmbm_mba_a_01')->where('san','=',$san)->get();
        if($has_row){
			
            $mark_update = DB::table('students_for_marks_rmbm_mba_a_01')
                ->where('san', $san)
                ->update(array($col => $val));
				
			$row = DB::table('students_for_marks_rmbm_mba_a_01')->where('san','=',$san)->first();
			...
			$mark_update = new StudentMarksRMBMMBAA01();
            $mark_update->san = $san;
	...
	public function excel_export(){
		...
		return Excel::create('Mark Input Sheet', function($excel) {
			...
			$excel->sheet('Mark Input Sheet', function($sheet) {
				...
				$sheet->loadView('export.marks_mba_rmbm_01');
	...
	public function to_word(){
		...
		//return Input::get('export_type');
			$student_data = DB::table('students_for_marks_rmbm_mba_a_01')
			->leftJoin('application_scj','students_for_marks_rmbm_mba_a_01.san','=','application_scj.san')
			->where('students_for_marks_rmbm_mba_a_01.san','=',$san)->get(); 
		if($student_data){
