...
class StudentMarksIMA01Controller extends \BaseController {
	...
	{
        return View::make('student_marks_IM_A_01');
	}


    public function students_for_marks_im_a_01(){
        //return Student::all();
    ...
    public function save_marks_for_IM_A_01(){
...
/*
        ...
        $has_row = DB::table('students_for_marks_im_a_01')->where('san','=',$san)->get();
        if($has_row){
			
            $mark_update = DB::table('students_for_marks_im_a_01')
                ->where('san', $san)
                ->update(array($col => $val));
				
			$row = DB::table('students_for_marks_im_a_01')->where('san','=',$san)->first();
			
			
			//Calculate marks
			
			 //print_r($row);
			if((($row->c1)>0)&&(($row->c2)>0)&&(($row->c3))&&(($row->c4))&&(($row->c5))&&(($row->c6))){
			...
			$mark_update = new StudentMarksIMA01();
            $mark_update->san = $san;
	...
	public function excel_export(){
		...
		return Excel::create('Mark Input Sheet', function($excel) {
			...
			$excel->sheet('Mark Input Sheet', function($sheet) {
				...
				$sheet->loadView('export.marks_ba_im_01');
	...
	public function to_word(){
		...
		//return Input::get('export_type');
			$student_data = DB::table('students_for_marks_im_a_01')
			->leftJoin('application_scj','students_for_marks_im_a_01.san','=','application_scj.san')
			->where('students_for_marks_im_a_01.san','=',$san)->get(); 
		if($student_data){
...
30% of the Final Mark<br>
  ...
  <tr>
    <td height="70">
	<strong>Range and use of secondary sources</strong>
	</td>
  ...
  <tr>
    <td height="70">
	<strong>Theoretical context </strong> – how well has the candidate applied their analysis to the Keller model?
</td>
  ...
  <tr>
    <td><strong>Brand context</strong> – how well has the candidate applied their work to the selected brand? </td>
     <td>&nbsp;</td>
	 <td>&nbsp;'.$student_data[0]->c3.'</td><td align="center">0.20</td>
    <td>&nbsp;'.($student_data[0]->c3*.20).'</td>
    
   
  </tr>
  <tr>
    <td><strong>Evaluation </strong>– a higher level student will identify limitations of their data, identify where assumptions are subjective, say where data is not available, evaluate their findings, critique the model, etc. </td>
     <td>&nbsp;</td>
	 <td>&nbsp;'.$student_data[0]->c4.'</td><td align="center">0.10</td>
    <td>&nbsp;'.($student_data[0]->c4*.10).'</td>
    
   
  </tr>
  <tr>
    <td><strong>Quality of presentation </strong>– use of the slides to analyse the brand, variety / style, evidence of working on analysis (both demonstrate awareness / understanding of contents), structure / organisation </td>
     <td>&nbsp;</td>
	 <td>&nbsp;'.$student_data[0]->c5.'</td><td align="center">0.20</td>
    <td>&nbsp;'.($student_data[0]->c5*.20).'</td>
    
   
  </tr>
  <tr>
    <td><strong>Communication of ideas </strong>– logical storytelling, professionalism, presentation skills, clarity of communication and clear ideas. </td>
     <td>&nbsp;</td>
	 <td>&nbsp;'.$student_data[0]->c6.'</td><td align="center">0.20</td>
    <td>&nbsp;'.($student_data[0]->c6*.20).'</td>
    
   
  </tr>
  <tr>
    <td colspan="4"><b>Weighted total</b></td>
    <td>&nbsp;'.(($student_data[0]->c3*.1)+($student_data[0]->c2*.2)+($student_data[0]->c1*.2)+($student_data[0]->c4*.1)+($student_data[0]->c5*.2)+($student_data[0]->c6*.2)).'</td>
  </tr>
  </table>
