...
class StudentMarksPMPMBAA01Controller extends \BaseController {
	...
	{
        return View::make('student_marks_PMP_MBA_A_01');
	}


    public function students_for_marks_pmp_mba_A_01(){
        //return Student::all();
        return DB::table('students')
            ->leftJoin('students_for_marks_pmp_mba_a_01','students.san','=','students_for_marks_pmp_mba_a_01.san')
    ...
    public function save_marks_for_pmp_mba_A_01(){
...
/*
        ...
        $has_row = DB::table('students_for_marks_pmp_mba_a_01')->where('san','=',$san)->get();
        if($has_row){
			
            $mark_update = DB::table('students_for_marks_pmp_mba_a_01')
                ->where('san', $san)
                ->update(array($col => $val));
				
			$row = DB::table('students_for_marks_pmp_mba_a_01')->where('san','=',$san)->first();
			...
			$mark_update = new StudentMarksPMPMBAA01();
            $mark_update->san = $san;
	...
	public function to_word(){
		...
		//return Input::get('export_type');
			$student_data = DB::table('students_for_marks_pmp_mba_a_01')
			->leftJoin('application_scj','students_for_marks_pmp_mba_a_01.san','=','application_scj.san')
			->where('students_for_marks_pmp_mba_a_01.san','=',$san)->get(); 
		if($student_data){
    ...
    "Content-Disposition"=>"attachment;Filename=pmp_".$student_data[0]->scj_number." & ".$student_data[0]->ls_student_number."_".Sentry::getUser()->first_name." ".Sentry::getUser()->last_name.".doc");
...
<table width="100%" align="center" border="1" cellspacing="1" cellpadding="2">
  ...
  <tr>
    <td colspan="5"><h2 style="text-align:center;">Postgraduate Major Project Feedback Sheet<br />
MBA<br />
MOD001170<br />
</h2></td>
  </tr>
