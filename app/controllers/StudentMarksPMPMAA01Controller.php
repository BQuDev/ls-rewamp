...
class StudentMarksPMPMAA01Controller extends \BaseController {
	...
	{
        return View::make('student_marks_pmp_ma_A_01');
	}


    public function students_for_marks_PMP_MA_A_01(){
        //return Student::all();
    ...
    public function save_marks_for_PMP_MA_A_01(){
...
/*
        ...
        $has_row = DB::table('students_for_marks_pmp_ma_a_01')->where('san','=',$san)->get();
        if($has_row){
			
            $mark_update = DB::table('students_for_marks_pmp_ma_a_01')
                ->where('san', $san)
                ->update(array($col => $val));
				
			$row = DB::table('students_for_marks_pmp_ma_a_01')->where('san','=',$san)->first();
			...
			$mark_update = new StudentMarksPMPMAA01();
            $mark_update->san = $san;
	...
	public function to_word(){
		...
		//return Input::get('export_type');
			$student_data = DB::table('students_for_marks_pmp_ma_a_01')
			->leftJoin('application_scj','students_for_marks_pmp_ma_a_01.san','=','application_scj.san')
			->where('students_for_marks_pmp_ma_a_01.san','=',$san)->get(); 
		if($student_data){
