...
class StudentMarksMCA01Controller extends \BaseController {
	...
	{
        return View::make('student_marks_MC_A_01');
	}


    public function students_for_marks_mc_a_01(){
        //return Student::all();
        return DB::table('students')
            ...
            ->select('students.san','sample','scj_number','students.ls_student_number','students.forename_1','c1','c2','m1','m2','ageed_mark')
            ->where('students.ls_student_number','>',0)
            ->groupBy('students.san')
            ->get();
    }

    public function save_marks_for_MC_A_01(){
...
/*
        ...
        $has_row = DB::table('students_for_marks_mc_a_01')->where('san','=',$san)->get();
        if($has_row){
			
            $mark_update = DB::table('students_for_marks_mc_a_01')
                ->where('san', $san)
                ->update(array($col => $val));
				
			$row = DB::table('students_for_marks_mc_a_01')->where('san','=',$san)->first();
			
			
			//Calculate marks
			
			 //print_r($row);
			if((($row->c1)>0)&&(($row->c2)>0)){
	...
	public function excel_export(){
		...
		return Excel::create('Mark Input Sheet', function($excel) {
			...
			$excel->sheet('Mark Input Sheet', function($sheet) {
				...
				$sheet->loadView('export.marks_ba_mc_01');
	...
	public function to_word(){
		...
		//return Input::get('export_type');
			$student_data = DB::table('students_for_marks_mc_a_01')
			->leftJoin('application_scj','students_for_marks_mc_a_01.san','=','application_scj.san')
			->where('students_for_marks_mc_a_01.san','=',$san)->get(); 
		if($student_data){
...
BA in Marketing<br />
MOD003058<br />
Assessment 1 –Individual Report<br /> 
...
•	To include an outline of the company<br> <br>
•	Their current position in the market <br><br>
•	Clearly identified client problem<br><br>
•	Implications for the business if the problem is not addressed<br>
  ...
  <tr>
    <td height="70">
	<strong>Situation analysis of the market: </strong><br>
•	To include a review of the market using suitable models (e.g. STEEPLE; SWOT McKinsey 7s, etc.) 

</td>
    <td>&nbsp;</td>  <td>&nbsp;'.$student_data[0]->c2.'</td><td align="center">0.60</td>
    <td>&nbsp;'.($student_data[0]->c2*.6).'</td>
    
  
  </tr>
  <tr>
    <td colspan="4"><b>Weighted total</b></td>
    <td>&nbsp;'.(($student_data[0]->c1*.4)+($student_data[0]->c2*.6)).'</td>
  </tr>
  </table>
