...
app.controller('MainCtrl', ['$scope', '$http', function ($scope, $http) {
  ...
  $scope.gridOptions.columnDefs = [
    ...
    { name: 'c1', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 1', cellTooltip: 
        function( row, col ) {
          return 'Title Initially this might be regarded as a working title, and ideally should mirror closely the content of the Introduction.';
        }, headerTooltip: 
        function( col ) {
          return 'Title Initially this might be regarded as a working title, and ideally should mirror closely the content of the Introduction.';
       }
    ...
    { name: 'c2', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 2' , cellTooltip: 
        function( row, col ) {
          return 'Introductory Part informs the reader of the problem or situation and the context you are interested in.goal is to give the reviewers some basic academic information they can use to evaluate the significance of the research proposal.';
        }, headerTooltip: 
        function( col ) {
			 return 'Introductory Part informs the reader of the problem or situation and the context you are interested in.goal is to give the reviewers some basic academic information they can use to evaluate the significance of the research proposal.';
        }
    ...
    { name: 'c3', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 3' , cellTooltip: 
        function( row, col ) {
          return 'Rationale for the studyDoes the document inform the reader of the rationale of this study?What is the research issue?Why is it an issue? Why is it an issue now?What could this research shed light on? ';
        }, headerTooltip: 
        function( col ) {
		 return 'Rationale for the studyDoes the document inform the reader of the rationale of this study?What is the research issue?Why is it an issue? Why is it an issue now?What could this research shed light on? ';
        }
    ...
    { name: 'c4', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 4' , cellTooltip: 
        function( row, col ) {
          return 'References / Format and Presentation identify previously conducted research';
        }, headerTooltip: 
        function( col ) {
		 return 'References / Format and Presentation identify previously conducted research';
      }
 ...
 $scope.gridOptions.onRegisterApi = function(gridApi){
          ...
          $.ajax({
                  type        : 'POST',
                  url         : 'save_marks_for_RMBM_MBA_A_01',
                  data        : formData,
                  dataType    : 'json',
                  success     : function(data) {
					  if((data == 1)&&(document.getElementById('auto_refresh').checked)){
						  $http.get('students_for_marks_RMBM_MBA_A_01')
            .success(function(data) {
    ...
    $scope.excel_export = function(){
					 ...
					 window.open("save_marks_for_RMBM_MBA_A_01_excel_export", '_blank');
...
today = dd+'-'+mm+'-'+yyyy;
	window.open("save_marks_for_RMBM_MBA_A_01_word?san="+Object.keys(san).map(function(k) { return san[k] })[0]+"&export_type=word&d="+today, '_blank');
	}
