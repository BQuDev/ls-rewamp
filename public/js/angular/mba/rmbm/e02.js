...
app.controller('MainCtrl', ['$scope', '$http', function ($scope, $http) {
  ...
  $scope.gridOptions.columnDefs = [
    ...
    { name: 'c1', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 1', cellTooltip: 
        function( row, col ) {
          return 'Introduction Have to introduce the topic and clarify the significance of what you are trying to present.Research aims ,Research objectives,Research questions.provide references.academic information';
        }, headerTooltip: 
        function( col ) {
          return 'Introduction Have to introduce the topic and clarify the significance of what you are trying to present.Research aims ,Research objectives,Research questions.provide references.academic information';
        }
    ...
    { name: 'c2', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 2' , cellTooltip: 
        function( row, col ) {
          return 'Literature Reviewknowledge of the literature and a critical link with the research question to be investigated.expected to critically review at least 6 sources.literature should mostly rely on published academic journal articles in the research area.This critical activity should produce a conceptual framework.';
        }, headerTooltip: 
        function( col ) {
			return 'Literature Reviewknowledge of the literature and a critical link with the research question to be investigated.expected to critically review at least 6 sources.literature should mostly rely on published academic journal articles in the research area.This critical activity should produce a conceptual framework.';
       }
    ...
    { name: 'c3', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 3' , cellTooltip: 
        function( row, col ) {
          return 'Research Design & MethodologyAppropriateness and rigour of research methods - Type of investigation,Data collection method, Sampling method, Accessibility issues, Ethical issues, Data analysis plan, Research limitations ';
        }, headerTooltip: 
        function( col ) {
		 return 'Research Design & MethodologyAppropriateness and rigour of research methods - Type of investigation,Data collection method, Sampling method, Accessibility issues, Ethical issues, Data analysis plan, Research limitations ';
        }
    ...
    { name: 'c4', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 4' , cellTooltip: 
        function( row, col ) {
          return 'Time Table Does provides a rationale time table?Do references correctly applied?should be written according to the module guide requirements, be spelled-checked and grammatically error free.';
        }, headerTooltip: 
        function( col ) {
		return 'Time Table Does provides a rationale time table?Do references correctly applied?should be written according to the module guide requirements, be spelled-checked and grammatically error free.';
       }
 ...
 $scope.gridOptions.onRegisterApi = function(gridApi){
          ...
          $.ajax({
                  type        : 'POST',
                  url         : 'save_marks_for_RMBM_MBA_A_02',
                  data        : formData,
                  dataType    : 'json',
                  success     : function(data) {
					  if((data == 1)&&(document.getElementById('auto_refresh').checked)){
						  $http.get('students_for_marks_RMBM_MBA_A_02')
            .success(function(data) {
    ...
    $scope.excel_export = function(){
					 ...
					 window.open("save_marks_for_RMBM_MBA_A_02_excel_export", '_blank');
...
today = dd+'-'+mm+'-'+yyyy;
	window.open("save_marks_for_RMBM_MBA_A_02_word?san="+Object.keys(san).map(function(k) { return san[k] })[0]+"&export_type=word&d="+today, '_blank');
	}
