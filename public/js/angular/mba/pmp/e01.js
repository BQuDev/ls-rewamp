...
app.controller('MainCtrl', ['$scope', '$http', function ($scope, $http) {
  ...
  $scope.gridOptions.columnDefs = [
    ...
    { name: 'c1', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 1', cellTooltip: 
        function( row, col ) {
          return 'Introduction adequate abstract? Clear purpose and rationale for the study? Clear set of objectives / research questions?';
        }, headerTooltip: 
        function( col ) {
    ...
    { name: 'c2', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 2' , cellTooltip: 
        function( row, col ) {
          return 'Use of Literature / Sources The range suitable and adequate? critical review of the literature been adopted? ‘link’ between existing literature and their own research?';
        }, headerTooltip: 
        function( col ) {
			return 'Use of Literature / Sources The range suitable and adequate? critical review of the literature been adopted? ‘link’ between existing literature and their own research?';
         }
    ...
    { name: 'c3', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 3' , cellTooltip: 
        function( row, col ) {
          return 'Research Design & Methodology research design and methodology discussed? approach appropriate?';
        }, headerTooltip: 
        function( col ) {
		  return 'Research Design & Methodology research design and methodology discussed? approach appropriate?';
         }
    ...
    { name: 'c4', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 4' , cellTooltip: 
        function( row, col ) {
          return 'Results, analysis and interpretation of data accurate data presented and analysed or descriptive? Appropriate theory applied? Interpretation –reasoned judgments on their findings?';
        }, headerTooltip: 
        function( col ) {
		  return 'Results, analysis and interpretation of data accurate data presented and analysed or descriptive? Appropriate theory applied? Interpretation –reasoned judgments on their findings?';
        }
    ...
    { name: 'c5', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 5' , cellTooltip: 
        function( row, col ) {
          return 'Conclusion & recommendations conclusions reasoned? correspond with the objective(s) reflected on the extent they have achieved their objectives? Limitations and future research discussed?';
        }, headerTooltip: 
        function( col ) {
		  return 'Conclusion & recommendations conclusions reasoned? correspond with the objective(s) reflected on the extent they have achieved their objectives? Limitations and future research discussed?';
          }
    ...
    { name: 'c6', type: 'number', enableFiltering: false,cellClass:'editable', displayName: 'Criteria 6' , cellTooltip: 
        function( row, col ) {
          return 'Presentationstructure & language, Harvard Referencing, appropriate of tables/diagrams?';
        }, headerTooltip: 
        function( col ) {
		  return 'Presentationstructure & language, Harvard Referencing, appropriate of tables/diagrams?';
        }
 ...
 $scope.gridOptions.onRegisterApi = function(gridApi){
          ...
          $.ajax({
                  type        : 'POST',
                  url         : 'save_marks_for_PMP_MBA_A_01',
                  data        : formData,
                  dataType    : 'json',
                  success     : function(data) {
					  if((data == 1)&&(document.getElementById('auto_refresh').checked)){
						  $http.get('students_for_marks_PMP_MBA_A_01')
            .success(function(data) {
    ...
    $scope.excel_export = function(){
					 ...
					 window.open("save_marks_for_PMP_MBA_A_01_excel_export", '_blank');
...
today = dd+'-'+mm+'-'+yyyy;
	window.open("save_marks_for_PMP_MBA_A_01_word?san="+Object.keys(san).map(function(k) { return san[k] })[0]+"&export_type=word&d="+today, '_blank');
	}
